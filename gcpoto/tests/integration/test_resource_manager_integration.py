...
def resource_manager_service(
    test_project_id: str, credentials_file: str
...
def organization_id() -> str:
    ...
    if not org_id:
        pytest.skip(
            "Organization ID not set. Set GCPOTO_TEST_ORG_ID to run folder tests."
        )
    return org_id
...
class TestResourceManagerIntegration:
    ...
    def test_list_projects(
        self, resource_manager_service: ResourceManagerService, test_project_id: str
    ):
...
class TestResourceManagerCreation:
    ...
    def test_create_update_delete_project(
        ...
        if not os.environ.get("GCPOTO_TEST_CREATE_PROJECT"):
            pytest.skip(
                "Project creation test skipped. Set GCPOTO_TEST_CREATE_PROJECT=1 to enable."
            )
        ...
        with register_resource_for_cleanup(
            resource_tracker, "project", project_id=test_project_name
        ):
            # Create a project
            ...
            # Verify the update
            assert (
                updated_project.display_name == f"Updated Project {test_project_name}"
            )
    ...
    def test_create_update_delete_folder(
        ...
        if not os.environ.get("GCPOTO_TEST_CREATE_FOLDER"):
            pytest.skip(
                "Folder creation test skipped. Set GCPOTO_TEST_CREATE_FOLDER=1 to enable."
            )
        ...
        try:
            ...
            updated_folder = resource_manager_service.update_folder(
                folder_id=folder.folder_id, display_name=updated_name
            )
            ...
            retrieved_updated_folder = resource_manager_service.get_folder(
                folder.folder_id
            )
            ...
            current_folders = resource_manager_service.list_folders(
                parent=f"organizations/{organization_id}"
            )
    ...
    def test_create_subfolder_hierarchy(
        self,
        resource_manager_service: ResourceManagerService,
        test_folder_name: str,
        organization_id: str,
        ...
        if not os.environ.get("GCPOTO_TEST_CREATE_FOLDER"):
            pytest.skip(
                "Folder hierarchy test skipped. Set GCPOTO_TEST_CREATE_FOLDER=1 to enable."
            )
        ...
        try:
            ...
            child_folders = resource_manager_service.list_folders(
                f"folders/{parent_folder_id}"
            )
    ...
    def test_move_project_between_folders(
        self,
        resource_manager_service: ResourceManagerService,
        test_project_name: str,
        test_folder_name: str,
        ...
        if not os.environ.get("GCPOTO_TEST_CREATE_PROJECT") or not os.environ.get(
            "GCPOTO_TEST_CREATE_FOLDER"
        ):
            pytest.skip(
                "Project move test skipped. Set both GCPOTO_TEST_CREATE_PROJECT=1 and GCPOTO_TEST_CREATE_FOLDER=1 to enable."
            )
        ...
        try:
            ...
            moved_project = resource_manager_service.move_project(
                project_id=project_id, new_parent=f"folders/{folder2_id}"
            )
    ...
    def test_list_folders_in_organization(
        self, resource_manager_service: ResourceManagerService, organization_id: str
    ):
        """Test listing folders in an organization."""
        # List folders in the organization
        folders = resource_manager_service.list_folders(
            parent=f"organizations/{organization_id}"
        )
