...
class TestResourceManagerEnhancedIntegration:
    ...
    def test_search_projects(
        ...
        if len(projects) == 0:
            pytest.skip(
                f"No projects found when searching for '{query}'. This could be due to permissions or API constraints."
            )
        else:
    ...
    def test_get_project_ancestors(
        self, resource_manager_service: ResourceManagerService, test_project_id: str
    ):
        ...
        try:
            ...
            for ancestor in ancestors:
                ...
                print(
                    f"- {ancestor['resourceType']}: {ancestor['displayName']} ({ancestor.get('id', '')})"
                )
    ...
    def test_get_project_iam_policy(
        self, resource_manager_service: ResourceManagerService, test_project_id: str
    ):
...
class TestFolderEnhancedIntegration:
    ...
    def test_search_folders(
        ...
        try:
            ...
            active_folders = resource_manager_service.search_folders(
                f"parent:organizations/{organization_id} state:ACTIVE"
            )
    ...
    def test_get_folder(
        ...
        try:
            ...
            if not folders:
                pytest.skip(
                    "No folders available to test get_folder - this could be due to insufficient permissions"
                )

            # Get the first folder
            sample_folder = folders[0]
            print(
                f"Found folder for testing: {sample_folder.display_name} (ID: {sample_folder.folder_id})"
            )
            ...
            assert (
                folder.display_name == sample_folder.display_name
            ...
            print(
                f"Successfully retrieved folder: {folder.display_name} (ID: {folder.folder_id})"
            )
    ...
    def test_get_folder_iam_policy(
        self, resource_manager_service: ResourceManagerService, organization_id: str
    ):
        ...
        try:
            ...
            # Get the IAM policy
            policy = resource_manager_service.get_folder_iam_policy(
                sample_folder.folder_id
            )
