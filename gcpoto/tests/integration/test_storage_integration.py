...
def test_object_content() -> bytes:
    ...
    return ''.join(random.choice(string.ascii_letters) for _ in range(10240)).encode('utf-8')
...
@pytest.fixture(scope="module")
def test_bucket(storage_service: StorageService, test_bucket_name: str, 
               common_tags: Dict[str, str], request) -> StorageBucket:
    ...
    def cleanup():
        try:
            ...
            for obj in objects:
                storage_service.delete_object(bucket_name=test_bucket_name, object_name=obj.name)
...
class TestStorageIntegration:
    ...
    def test_create_get_bucket_with_tags(self, storage_service: StorageService, 
    ...
    def test_object_upload_download(self, storage_service: StorageService,
                                  test_bucket: StorageBucket,
                                  test_bucket_name: str,
        ...
        object_tags = {**common_tags, "content-type": "text/plain", "test-case": "upload-download"}
        ...
        obj_meta = storage_service.get_object(bucket_name=test_bucket_name, object_name=test_object_name)
    ...
    def test_object_delete(self, storage_service: StorageService,
                         test_bucket_name: str,
                         test_object_name: str):
    ...
    def test_upload_with_metadata(self, storage_service: StorageService,
                                test_bucket_name: str,
                                test_file: str,
                                common_tags: Dict[str, str]):
        ...
        obj = storage_service.get_object(bucket_name=test_bucket_name, object_name=object_name)
        ...
        # Clean up
        storage_service.delete_object(bucket_name=test_bucket_name, object_name=object_name)
    
    def test_list_objects_with_prefix(self, storage_service: StorageService,
                                    test_bucket_name: str,
                                    test_file: str,
                                    common_tags: Dict[str, str]):
        ...
        assert all(obj.name.startswith(f"{prefix}/subfolder/") for obj in subfolder_objects)
        
        # Clean up
        for obj_name in object_names:
            storage_service.delete_object(bucket_name=test_bucket_name, object_name=obj_name)
