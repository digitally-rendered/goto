...
def test_project_id() -> str:
    ...
    """
    project_id = os.environ.get("GCPOTO_TEST_PROJECT_ID") or os.environ.get(
...
def credentials_file() -> str:
    ...
    """
    return os.environ.get("GCPOTO_TEST_CREDENTIALS") or os.environ.get(
...
@pytest.fixture(scope="session")
def storage_service(
    test_project_id: str, credentials_file: str, mock_mode: bool
) -> StorageService:
    ...
    service = StorageService(
        project_id=test_project_id, credentials_file=credentials_file
    )
...
class ResourceTracker:
    ...
    billing_budgets: List[Dict[str, str]] = field(
        default_factory=list
    )  # [{'account_id': 'id', 'budget_id': 'id'}]
    ...
    def register_budget(self, billing_account_id: str, budget_id: str) -> None:
        ...
        self.billing_budgets.append(
            {"account_id": billing_account_id, "budget_id": budget_id}
        )
    ...
    def cleanup_all(
        ...
        for subscription_name in self.pubsub_subscriptions:
            try:
                ...
                error_msg = (
                    f"Failed to clean up subscription {subscription_name}: {str(e)}"
                )
                logging.error(error_msg)
                cleanup_errors.append(error_msg)
        ...
        for budget in self.billing_budgets:
            try:
                ...
                logging.info(
                    f"Cleaned up budget: {budget['account_id']}/{budget['budget_id']}"
                )
            except Exception as e:
...
def billing_service(
    ...
    if mock_mode:
        ...
        class MockProjectBillingInfo:
            def __init__(self, project_id, billing_enabled=True):
                ...
                self.billing_account_name = (
                    "billingAccounts/ABCDEF-123456-789012" if billing_enabled else ""
                )
        ...
        service.cloud_billing_client.get_project_billing_info.return_value = (
            mock_project_info
        )

        # Setup mock listings for projects in a billing account
        service.cloud_billing_client.list_project_billing_info.return_value = [
            mock_project_info
        ]
...
def ensure_services_enabled(test_project_id: str):
    ...
    try:
        ...
        for service in service_usage_client.list_services(
        ...
        print(
            f"Warning: Failed to list enabled services for {test_project_id}: {e}. Proceeding may lead to enable errors."
        )
        # Initialize as empty if listing failed, forcing enable attempts for all required services
        enabled_services = set()

    services_to_enable = [s for s in required_services if s not in enabled_services]
    ...
    for service_name in services_to_enable:
        try:
            ...
            request = EnableServiceRequest(name=f"{parent}/services/{service_name}")
            operation = service_usage_client.enable_service(request=request)
            enable_operations[service_name] = operation
            print(
                f"Enable operation started for {service_name}: {operation.operation.name}"
            )
            ...
            print(
                f"Warning: Failed precondition enabling {service_name}: {e}. Might already be in process."
            )
        except google_exceptions.NotFound as e:
            print(
                f"Warning: Service {service_name} not found for project {test_project_id}: {e}. Skipping."
            )
        except Exception as e:
            pytest.fail(
                f"Failed to start enable operation for {service_name}: {e}",
                pytrace=False,
            )
        ...
        for service_name in list(enable_operations.keys()):
            ...
            if not op_done:
                ...
                try:
                    ...
                    current_op_state = service_usage_client.get_operation(
                    ...
                    print(
                        f"Warning: Error checking operation status for {service_name}: {type(e).__name__} - {e}"
                    )
            ...
            if op_done:
                ...
                # Case 2: The operation was already completed (from initial done() check)
                    ...
                    )  # Direct check on initial op object
                    if op_error:
                        pytest.fail(
                        ...
                        if (
                            service_name in enable_operations
                        ):  # Ensure it wasn't already deleted
                            print(
                                f"Service {service_name} enablement already completed."
                            )
    ...
    if not operations_complete:
        ...
        pytest.fail(
            f"Timeout waiting for services to enable: {remaining}", pytrace=False
        )
...
@contextmanager
def register_resource_for_cleanup(
    resource_tracker: ResourceTracker, resource_type: str, **resource_data
):
    ...
    try:
        ...
        logging.error(
            f"Exception while testing with {resource_type} {resource_id}: {str(e)}"
        )
        raise
