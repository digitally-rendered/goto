...
def billing_service(test_project_id: str, credentials_file: str) -> BillingService:
    ...
    """
    return BillingService(project_id=test_project_id, credentials_file=credentials_file)
...
class TestBillingEnhancedIntegration:
    ...
    def test_get_billing_account(self, billing_service: BillingService):
        ...
        print(
            f"Successfully retrieved billing account: {account.display_name} ({account.billing_account_id})"
        )
    ...
    def test_list_project_billing_info(
        ...
        try:
            projects = billing_service.list_project_billing_info(
                sample_account.billing_account_id
            )
            ...
            if projects:
                ...
                print(
                    f"Found {len(projects)} projects associated with billing account {sample_account.billing_account_id}"
                )
                for project in projects[
                    :5
                ]:  # Just print first 5 to avoid too much output
                    print(
                        f"- Project: {project.project_id}, Billing enabled: {project.billing_enabled}"
                    )
    ...
    def test_enable_disable_project_billing(
        self, billing_service: BillingService, test_project_id: str
    ):
    ...
    def test_get_sku_pricing(self, billing_service: BillingService):
        ...
        try:
            ...
            assert (
                "pricing_info" in pricing
                or "category" in pricing
                or "service_regions" in pricing
            )
    ...
    def test_search_billing_accounts(self, billing_service: BillingService):
        ...
        assert all(
            a.open is True for a in open_accounts
        ), "Found closed accounts when filtering for open only"
...
class TestBillingBudgetEnhancedIntegration:
    ...
    def test_get_budget(self, billing_service: BillingService, billing_account_id: str):
        ...
        try:
            ...
            # Now test get_budget
            budget = billing_service.get_budget(
                billing_account_id, sample_budget.budget_id
            )
