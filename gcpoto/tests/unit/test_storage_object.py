...
def test_list_objects(mock_storage_service, sample_object_response):
    ...
    mock_storage_service.service.objects().list.assert_called_with(bucket="test-bucket", prefix="test/")
...
def test_download_object(mock_storage_service, tmp_path):
    ...
    mock_storage_service.service._http.MediaIoBaseDownload.return_value = mock_downloader
...
def test_upload_object(mock_storage_service, sample_object_response, tmp_path):
    ...
    with mock.patch("googleapiclient.http.MediaInMemoryUpload") as mock_upload:
        ...
        obj = service.upload_object("test-bucket", "test-object.txt", content, "text/plain")
    ...
    with mock.patch("googleapiclient.http.MediaInMemoryUpload") as mock_upload:
        ...
        obj = service.upload_object(
            "test-bucket", "test-object.txt", content,
            metadata={"key1": "value1"}, cacheControl="public, max-age=3600"
        )
