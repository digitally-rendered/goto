...
# Mock the entire billing_budgets_v1 module and its types submodule
@pytest.fixture(scope="function", autouse=True)
def mock_billing_budgets_module():
    """Mock the google.cloud.billing_budgets_v1 module and its types."""
    # Create mock types for Budget, ThresholdRule, and NotificationsRule
    mock_budget_class = MagicMock()
    mock_threshold_rule_class = MagicMock()
    mock_notifications_rule_class = MagicMock()

    # Create a mock types module containing these classes
    mock_types_module = MagicMock()
    mock_types_module.Budget = mock_budget_class
    mock_types_module.ThresholdRule = mock_threshold_rule_class
    mock_types_module.NotificationsRule = mock_notifications_rule_class

    # Create the main module with BudgetServiceClient and types
    mock_module = MagicMock()
    mock_module.types = mock_types_module
    mock_module.BudgetServiceClient = MagicMock

    # Apply the mocks to the module imports
    with patch.dict(
        "sys.modules",
        {
            "google.cloud.billing_budgets_v1": mock_module,
            "google.cloud.billing_budgets_v1.types": mock_types_module,
        },
    ):
        yield mock_module
...
@pytest.fixture
def mock_budget_client(mock_billing_budgets_module):
    """Create a mock BudgetServiceClient."""
    mock_client = MagicMock()
    mock_billing_budgets_module.BudgetServiceClient.return_value = mock_client
    return mock_client


@pytest.fixture
def mock_billing_service(
    mock_cloud_billing_client, mock_cloud_catalog_client, mock_budget_client
):
    """Create a BillingService with mocked clients."""
    with patch(
        "gcpoto.services.billing.BillingService._proto_to_dict"
    ) as mock_proto_to_dict:
        # Configure the mock proto_to_dict to return a complete dict with all required fields
        mock_proto_to_dict.side_effect = lambda proto: json.loads(
            json.dumps(
                {
                    "name": getattr(proto, "name", ""),
                    "display_name": getattr(proto, "display_name", ""),
                    "budget_id": getattr(proto, "budgetId", ""),
                    "billing_account_id": getattr(proto, "billing_account_id", ""),
                    "type": getattr(proto, "type", "billing.budget"),
                    "amount": {
                    ...
                    "budget_filter": getattr(
                        proto, "budget_filter", {"projects": ["projects/test-project"]}
                    ),
                    "labels": getattr(proto, "labels", {"department": "engineering"}),
                    "project": "test-project",
                    "id": getattr(proto, "name", ""),
                }
            )
        )
...
def sample_budget_proto():
    ...
    budget.__dict__["amount"] = {
        "specified_amount": {"currency_code": "USD", "units": "1000"}
    }
    budget.__dict__["threshold_rules"] = [
        {"threshold_percent": 0.9},
        {"threshold_percent": 1.0},
    ]
    budget.__dict__["budget_filter"] = {"projects": ["projects/test-project"]}

    # Add attributes required for Pydantic V2 validation
    budget.budgetId = "budget-12345678"
    budget.billing_account_id = "ABCDEF-123456"
    budget.type = "billing.budget"
    budget.labels = {"department": "engineering"}
...
class TestBillingServiceBudgets:
    ...
    def test_list_budgets(
        self, mock_billing_service, sample_budget_proto, mock_budget_client
    ):
        ...
        mock_budget_client.list_budgets.assert_called_once_with(
            parent="billingAccounts/ABCDEF-123456"
        )

        # Test with already formatted name
        mock_billing_service.list_budgets("billingAccounts/ABCDEF-789012")
        mock_budget_client.list_budgets.assert_called_with(
            parent="billingAccounts/ABCDEF-789012"
        )
    ...
    def test_get_budget(
        ...
        # Verify client was called correctly
        mock_budget_client.get_budget.assert_called_once_with(
            name="billingAccounts/ABCDEF-123456/budgets/budget-12345678"
        )

        # Test with already formatted billing account name
        mock_billing_service.get_budget(
            "billingAccounts/ABCDEF-789012", "budget-12345678"
        )
        mock_budget_client.get_budget.assert_called_with(
            name="billingAccounts/ABCDEF-789012/budgets/budget-12345678"
        )
    ...
    def test_create_budget(
        ...
        assert (
            mock_budget_client.create_budget.call_args[1]["parent"]
            == "billingAccounts/ABCDEF-123456"
        )
    ...
    def test_update_budget(
        ...
        assert (
            budget.display_name == "Test Budget"
        )  # It's mocked to return the original proto
        ...
        budget = mock_billing_service.update_budget(
            billing_account_id="ABCDEF-123456", budget_id="budget-12345678"
        )
    ...
    def test_delete_budget(self, mock_billing_service, mock_budget_client):
        ...
        # Test with already formatted billing account name
        mock_billing_service.delete_budget(
            "billingAccounts/ABCDEF-789012", "budget-87654321"
        )
    ...
    def test_proto_to_dict(self, mock_billing_service):
        ...
        # Instead of using __get__, directly implement the method for testing
        def proto_to_dict_impl(proto):
            # Simple implementation of the proto_to_dict method for testing
            try:
                if hasattr(proto, "to_json"):
                ...
                print(f"Error converting proto to dict: {e}")
                return {}

        # Assign the implementation to the mock
        mock_billing_service._proto_to_dict = proto_to_dict_impl
