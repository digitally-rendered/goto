...
def mock_budget_client():
    ...
    with patch(
        "google.cloud.billing_budgets_v1.BudgetServiceClient", return_value=mock_client
    ):
        yield mock_client
...
class TestBillingServiceAccounts:
    ...
    def test_list_billing_accounts(
        self, mock_billing_service, sample_billing_account_proto
    ):
        """Test listing billing accounts."""
        # Setup
        billing_client = mock_billing_service.cloud_billing_client
        billing_client.list_billing_accounts.return_value = [
            sample_billing_account_proto
        ]
        ...
        billing_client.list_billing_accounts.assert_called_once_with(
            request=billing_v1.ListBillingAccountsRequest(filter="")
        )
        ...
        # We need to check that the second call was made with a filter for open accounts
        expected_request = billing_v1.ListBillingAccountsRequest(filter="open = true")
        billing_client.list_billing_accounts.assert_called_with(
    ...
    def test_get_billing_account(
        ...
        # Verify client was called correctly
        billing_client.get_billing_account.assert_called_once_with(
            name="billingAccounts/ABCDEF-123456"
        )

        # Test with already formatted name
        mock_billing_service.get_billing_account("billingAccounts/ABCDEF-789012")
        billing_client.get_billing_account.assert_called_with(
            name="billingAccounts/ABCDEF-789012"
        )
    ...
    def test_get_project_billing_info(
        self, mock_billing_service, sample_project_billing_info_proto
    ):
        """Test getting project billing info."""
        # Setup
        billing_client = mock_billing_service.cloud_billing_client
        billing_client.get_project_billing_info.return_value = (
            sample_project_billing_info_proto
        )
        ...
        # Verify client was called correctly
        billing_client.get_project_billing_info.assert_called_once_with(
            name="projects/test-project"
        )

        # Test with already formatted name
        mock_billing_service.get_project_billing_info("projects/another-project")
        billing_client.get_project_billing_info.assert_called_with(
    ...
    def test_update_project_billing_info(
        ...
        # Setup
        billing_client = mock_billing_service.cloud_billing_client
        billing_client.update_project_billing_info.return_value = (
            sample_project_billing_info_proto
        )

        # Execute - test adding billing account
        info = mock_billing_service.update_project_billing_info(
            project_id="test-project", billing_account_id="ABCDEF-123456"
        )
        ...
        assert (
            call_args["project_billing_info"].billing_account_name
        ...
        mock_billing_service.update_project_billing_info(
            project_id="test-project", billing_account_id=None
        )
        ...
        # Test error handling
        billing_client.update_project_billing_info.side_effect = Exception(
            "Update failed"
        )
    ...
    def test_list_project_billing_info(
        ...
        # Setup
        billing_client = mock_billing_service.cloud_billing_client
        billing_client.list_project_billing_info.return_value = [
            sample_project_billing_info_proto
        ]
        ...
        # Verify client was called correctly
        billing_client.list_project_billing_info.assert_called_once_with(
            name="billingAccounts/ABCDEF-123456"
        )

        # Test with already formatted name
        mock_billing_service.list_project_billing_info("billingAccounts/ABCDEF-789012")
        billing_client.list_project_billing_info.assert_called_with(
            name="billingAccounts/ABCDEF-789012"
        )
    ...
    def test_list_skus(self, mock_billing_service):
        ...
        assert (
            skus[0]["description"] == "N1 Predefined Instance Core running in Americas"
        )
        ...
        # Verify client was called correctly
        catalog_client.list_skus.assert_called_once_with(
