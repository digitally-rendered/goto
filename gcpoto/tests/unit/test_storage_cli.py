...
def test_list_objects_cli(cli_runner):
    ...
    with mock.patch(
        "gcpoto.services.storage.StorageService", autospec=True
    ) as mock_service_cls:
...
def test_get_object_cli(cli_runner, sample_storage_object):
    ...
    with (
        ...
        assert (
            kwargs.get("output_format", args[1] if len(args) > 1 else "json") == "json"
        )  # Default format
...
def test_download_object_cli(cli_runner):
    ...
    with mock.patch(
        ...
        # Verify the service was called with the right parameters
        mock_service.download_object.assert_called_with(
            "test-bucket", "test-object.txt", "test-object.txt"
        )
        ...
        mock_service.download_object.assert_called_with(
            "test-bucket", "test-object.txt", "custom-path.txt"
        )
...
def test_upload_object_cli(cli_runner, sample_storage_object):
    ...
    with (
        ...
        mock_service.upload_object.assert_called_with(
            "test-bucket", "test-file.txt", "local-file.txt", None
        )
