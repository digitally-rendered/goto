...
from gcpoto.models.pubsub import (
    PubSubTopic,
    PubSubSubscription,
...
def test_pubsub_topic_model_creation():
    ...
    storage_policy = MessageStoragePolicy(
    ...
    assert topic.message_storage_policy.allowed_persistence_regions == [
        "us-central1",
        "us-east1",
    ]
    ...
    assert json_data["message_storage_policy"]["allowed_persistence_regions"] == [
        "us-central1",
        "us-east1",
    ]
...
def test_pubsub_subscription_model_creation():
    ...
    push_config = PushConfig(
        push_endpoint="https://example.com/push", attributes={"x-goog-version": "v1"}
    )
    dead_letter_policy = DeadLetterPolicy(
        dead_letter_topic="dead-letter", max_delivery_attempts=5
    )
