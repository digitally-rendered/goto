...
def mock_google_client():
    ...
    with mock.patch("googleapiclient.discovery.build") as mock_build:
        ...
        mock_service.projects.return_value.subscriptions.return_value = (
            mock_subscriptions
        )

        # Set up projects().topics().subscriptions() chain
        mock_topic_subscriptions = mock.MagicMock()
        mock_service.projects.return_value.topics.return_value.subscriptions.return_value = (
            mock_topic_subscriptions
        )
...
def test_list_resources(mock_google_client, sample_topic_response):
    ...
    mock_list_next = (
        mock_google_client.projects.return_value.topics.return_value.list_next
    )
...
def test_create_topic(mock_google_client, sample_topic_response):
    ...
    mock_create.assert_called_with(
        name="projects/test-project/topics/test-topic", body={}
    )

    # Test with conflict error
    mock_request.execute.side_effect = HttpError(
        resp=mock.MagicMock(status=409), content=b"Topic already exists"
    )
...
def test_publish_message(mock_google_client, mock_publisher_client):
    ...
    message_id = service.publish_message(
        topic_name="test-topic", data="test message", attributes={"key": "value"}
    )

    # Verify the publisher client was called correctly
    mock_publisher_client.publish.assert_called_once_with(
        "projects/test-project/topics/test-topic", b"test message", key="value"
    )
    ...
    mock_publisher_client.publish.assert_called_with(
        "projects/test-project/topics/test-topic", binary_data
    )

    # Test with full path
    service.publish_message("projects/test-project/topics/test-topic", "test message")
    mock_publisher_client.publish.assert_called_with(
        "projects/test-project/topics/test-topic", b"test message"
    )


def test_list_subscriptions_by_project(
    mock_google_client, sample_subscription_response
):
    ...
    # Mock list_next for pagination
    mock_list_next = (
        mock_google_client.projects.return_value.subscriptions.return_value.list_next
    )
    mock_list_next.side_effect = [mock.MagicMock(), None]
...
def test_list_subscriptions_by_topic(mock_google_client, sample_subscription_response):
    ...
    mock_request = mock.MagicMock()
    mock_list = (
        mock_google_client.projects.return_value.topics.return_value.subscriptions.return_value.list
    )
    mock_list.return_value = mock_request

    # Mock list_next for pagination
    mock_list_next = (
        mock_google_client.projects.return_value.topics.return_value.subscriptions.return_value.list_next
    )
    mock_list_next.side_effect = [mock.MagicMock(), None]
    ...
    mock_get.assert_called_with(
        subscription="projects/test-project/subscriptions/test-subscription-2"
    )
...
def test_create_subscription(mock_google_client, sample_subscription_response):
    ...
    mock_request = mock.MagicMock()
    mock_create = (
        mock_google_client.projects.return_value.subscriptions.return_value.create
    )
    mock_create.return_value = mock_request
    mock_request.execute.return_value = sample_subscription_response
    ...
    # Test with conflict error
    mock_request.execute.side_effect = HttpError(
        resp=mock.MagicMock(status=409), content=b"Subscription already exists"
    )

    with pytest.raises(
        ValueError, match="Subscription 'test-subscription' already exists"
    ):
...
def test_delete_subscription(mock_google_client):
    ...
    mock_request = mock.MagicMock()
    mock_delete = (
        mock_google_client.projects.return_value.subscriptions.return_value.delete
    )
    mock_delete.return_value = mock_request
    mock_request.execute.return_value = {}
...
def test_pull_messages(mock_google_client):
    ...
    mock_request.execute.return_value = {"receivedMessages": mock_messages}

    # Create service and call pull_messages
    service = PubSubService(project_id="test-project")
    messages = service.pull_messages(
        "test-subscription", max_messages=5, return_immediately=True
    )

    # Verify the API call was made correctly
    mock_pull.assert_called_once_with(
        subscription="projects/test-project/subscriptions/test-subscription",
        body={"maxMessages": 5, "returnImmediately": True},
    )
...
def test_acknowledge_messages(mock_google_client):
    ...
    mock_request = mock.MagicMock()
    mock_ack = (
        mock_google_client.projects.return_value.subscriptions.return_value.acknowledge
    )
