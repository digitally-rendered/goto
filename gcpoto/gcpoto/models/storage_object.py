...
class StorageObject(GCPResource):
    ...
    content_type: Optional[str] = Field(
        None, description="The content type of the object data"
    )
    ...
    generation: Optional[str] = Field(
        None, description="The content generation of this object"
    )
    md5_hash: Optional[str] = Field(None, description="MD5 hash of the object data")
    crc32c: Optional[str] = Field(
        None, description="CRC32c checksum of the object data"
    )
    ...
    cache_control: Optional[str] = Field(
        None, description="Cache control for the object data"
    )
    metadata: Optional[Dict[str, str]] = Field(
        None, description="User-provided metadata for the object"
    )
    acl: Optional[List[ObjectAclEntry]] = Field(
        None, description="Access control list for the object"
    )
    ...
    def from_api_response(cls, response: Dict[str, Any]) -> "StorageObject":
        ...
        if response.get("acl"):
            acl_entries = [
                ObjectAclEntry(
                    entity=entry.get("entity", ""), role=entry.get("role", "")
                )
